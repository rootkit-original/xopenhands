...
def open_browser_after_delay(url: str, delay: int = 5) -> None:
    """Open the browser after a delay to ensure the server is ready.

    Args:
        url: The URL to open in the browser
        delay: Number of seconds to wait before opening the browser
    """
    ...
    def _open():
        ...
        try:
            ...
            print_formatted_text("")
            print_formatted_text(
                HTML(f"<ansigreen>🌐 Opening browser at {url}</ansigreen>")
            )
        except Exception as e:
            # If browser opening fails, don't crash the server
            print_formatted_text(
                HTML(
                    f"<ansiyellow>⚠️ Could not automatically open browser: {e}</ansiyellow>"
                )
            )
...
def launch_gui_server(mount_cwd: bool = False, gpu: bool = False) -> None:
    ...
    # Start a background thread to open the browser after the server starts
    open_browser_after_delay("http://localhost:3000", delay=5)
